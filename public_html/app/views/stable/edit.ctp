...
function moreimg() {
	...
	function listuser() {
		...
		if(userid) {
			document.getElementById("listhrse").innerHTML="<font color=#FF00000>Please Wait.................. </font>";
			new Ajax.Updater('listhrse','<?php echo $html->url('/stable/listhorse'); ?>'+'/'+userid, {asynchronous:true, evalScripts:true});
		}
		else {
			document.getElementById("listhrse").innerHTML="<font color=#FF00000>Select User </font>";
		}
	}	
	function liststate() {
						...
						<select name="data[Stable][userid]" class="look" id="user" >
							...
							if(is_array($user_arr)) {
								foreach($user_arr as $key=>$val) :
									if($val['User']['id']==$edit_arr['Stable']['userid']) {
										...
										if(count($listhorst)>0) {
											foreach($listhorst as $key=>$val) :
												...
												//if($val['Horse']['stable_id']>0) {
													if($val['Horse']['stable_id']==$Stableid) {?>
...
style="width: 10px;" class="jScrollPaneDragBottom"></div></div></div><div
 style="height: 0px;" class="jScrollCap jScrollCapBottom"></div></div>
						 <br><span class="error"><?php echo $form->error('Stable.horse_id');?>
						 </span>					</td>
				 </tr>
				 ...
				 <tr class="even_tr" style="padding:10px;">
					...
					<td width="70%" align="left" valign="top" class="smalltext">
						 <?php echo $form->text('Stable.stable_name',array("class"=>"look","size"=>"30","value"=>$edit_arr['Stable']['stable_name'])); ?>
				 ...
				 <tr class="even_tr" style="padding:10px;">
								...
								<select name="data[Horse][countryid]" class="dropdown" id="HorseCountry" onChange="liststate()">
									...
									if(is_array($country_arr)) {
										foreach($country_arr as $key=>$val) :	
											if($val['Country']['id']==$edit_arr['Stable']['country_id']) {
												$sel="selected=selected";
											}
											else {
												$sel="";
				  ...
				  <tr class="even_tr" style="padding:10px;">
					...
					<td width="70%" align="left" valign="top" class="smalltext">
						 <?php echo $form->textarea('Stable.about',array("class"=>"look","rows"=>"8","cols"=>"82","value"=>$edit_arr['Stable']['about'])); ?>
				  ...
				  <tr class="even_tr" style="padding:10px;">
					...
					<td width="70%" align="left" valign="top" class="smalltext">
						 <?php echo $form->textarea('Stable.services',array("class"=>"look","rows"=>"8","cols"=>"82","value"=>$edit_arr['Stable']['services'])); ?>
				 ...
				 <tr class="even_tr" style="padding:10px;">
					...
					<td width="70%" align="left" valign="top" class="smalltext">
							<?php
							if($edit_arr['Stable']['stable_image']!="") {
								e("<img src=".$this->webroot."img/stable_image/".$edit_arr['Stable']['stable_image']." width=160 border=0 class=wrap>");
							}
							?>
							<?php	
