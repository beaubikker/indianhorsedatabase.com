...
<script language="javascript">
	function cancelmembership() {
		if(confirm("Are you sure to cancel your premuim membership?If yes then your premuim membership will be cancelled ")) {
			window.location.href='<?php e($html->url('/user/cancelpremiummembership'));?>'		
		}	
	}
	function frmsub() {
	...
	<div id="wrapper_parrent">		
		...
		<div class="sub_body">			
				<div class="upper">
					...
					<form action="<?php e($html->url('/user/paymentformemshipexpiration'));?>" method="post" name="updatemembership">			
					<div class="profile_info">
								<h2 class="header2">
								<?php
								if($userarr['User']['payment_status']=='N') {
										e("<br>You have not paid for the service");
								}
								else {
								?>
									Activate Your Membership
									<br>(Your membership has  expired on <?php e(date('d F Y', strtotime($userarr['User']['membership_exipre_date']))); ?>)
								<?php
								}
								?>
								</h2>
								...
								<img src="<?php e($this->webroot);?>img/paypal.jpeg" style="cursor:pointer"  onClick="frmsub()">																						
									&nbsp;&nbsp;	
								<input class="submit_btn1" type="button" value="Cancel"  onClick="cancelmembership()" style="cursor:pointer"/>
								</div>											
					</div>	
					</form>	
					</div>
