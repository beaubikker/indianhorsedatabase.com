...
<script language="javascript">
	...
	function showsalehorse(id) {
		document.getElementById("salehorse_"+id).style.display="block";	
	}
	function noshowsalehorse(id) {
		document.getElementById("salehorse_"+id).style.display="none";	
	}
	...
	function subscribefornotification(user_id) {
		if(user_id) {
			//document.getElementById("subsmsg").innerHTML="<font color=#FF0000>Please wait....<br></font>";
			...
			if(req)
			{
				...
				req.open("GET","<?php echo $html->url('/user/subscribefornotification/');?>"+user_id,true);	
				req.send(null);					
			}
		}
	}
	...
	function unsubsubscribefornotification(user_id) {
		if(user_id) {
			//document.getElementById("subsmsg").innerHTML="<font color=#FF0000>Please wait....<br></font>";
			...
			{
				req.onreadystatechange=processrequesnoti;
				req.open("GET","<?php echo $html->url('/user/unsubscribefornotification/');?>"+user_id,true);	
				req.send(null);					
			}
		}	
	}
			  ...
			  <div>&nbsp;</div>			  	
					...
					<div class="profile_info">							
						...
						<div class="po_inf_mid">
							<div style="position: absolute; right: 50px;top: 285px; text-align: right;">								
							...
							if(is_numeric($chksession)) {
								if($usertype=="P") {
							?>
								<div class="big4" align="right" style="padding-left:600px">									
								<?php	
									if(count($chk_arr)>0) {
								?>	
										<span id="unsub" style="display:block;"><img src="<?php e($this->webroot);?>img/submit_btndn.png" border="0" onClick="unsubsubscribefornotification('<?php e($userid);?>')" style="cursor:pointer"></span>
										<span id="subs" style="display:none;">
											<img src="<?php e($this->webroot);?>img/submit_btnup.png" border="0" onClick="subscribefornotification('<?php e($userid);?>')" style="cursor:pointer">
										</span>
									<?php
									}
									else {
									?>
										<span id="unsub" style="display:none;"><img src="<?php e($this->webroot);?>img/submit_btndn.png" border="0" onClick="unsubsubscribefornotification('<?php e($userid);?>')" style="cursor:pointer"></span>
								...
								<div class="horse2">																		
									<div class="form_box">
									  ...
									  ?>
										<div class="form_box">
											...
											<span><a  href="<?php e($html->url('/stable/viewprofile/'.$stablenamearr[0]['Stable']['id']));?>" style="color:#994F26"><?php e($stablenamearr[0]['Stable']['stable_name']);?></a></span>	
											<div class="clear"></div>										
										</div>
									<?php
									}
									?>
										...
										<div class="form_box">
											...
											<span><a href="<?php e($http);?><?php e($userarr['User']['website']);?>" target="_blank"><?php e($userarr['User']['website']);?></a></span>	
											<div class="clear"></div>
										</div>										
										<?php
										...
										?>	
										<div class="form_box">
											<label class="formarea">Facebook :</label>
											<span><a href="<?php e($userarr['User']['facebook_url'])?>" target="_blank"><img src="<?php e($this->webroot);?>img/facebook.png" alt=""  style="cursor:pointer" border=""/></a></span>	
											<div class="clear"></div>
										</div>		
										<?php
										}
										...
										<div class="form_box47">
											...
											<span class="nio"><?php e($userarr['User']['about_me']);?></span>
										</div>
								</div>
							</div>	
						  <div class="classified" style="padding-left:6px">
								<?php
								if(count($ownerhorse_arr)>0) {
								?>
									<input  type="button"  value="Owned by <?php e($userarr['User']['firstname']);?>"   class="newbutton" />								
								<?php
								}
								?>
								<ul>
									...
									if(count($ownerhorse_allarr)>=8) {
										?>
											<br><br>
											<div align="center"><a class="viewallowned" href="javascript:void(0)" ><input  type="button"  value="View All"   class="newbutton"  style="cursor:pointer"/></a></div>
										<?php
									}
									?>						
								</ul>
								...
								if(count($breddnamearr)>0) {?>
									<input  type="button"  value="Bred by <?php e($userarr['User']['firstname']);?>"   class="newbutton" />
									<?php
								}
								?>	
								...
								<ul>
									...
									if(count($allbreddnamearr)>=8) {
										?>
											<br><br>
											<div align="center"><a class="viewallbred" href="javascript:void(0)" ><input  type="button"  value="View All"   class="newbutton" style="cursor:pointer"/></a></div>
										<?php
									}
									?>								
								</ul>							
								...
								if(count($listhorsesalearr)>0) {?>
									<input  type="button"  value="Horses for sale"   class="newbutton" />
									<?php
								}
								?>								
								<ul>
