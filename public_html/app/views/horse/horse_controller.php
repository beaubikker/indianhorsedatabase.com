...
class HorseController  extends AppController
{
	...
	function salehorse() {
		...
		//if(isset($_GET['search'])) {
			...
			$sql="SELECT * FROM tbl_horses Horse LEFT JOIN tbl_horsesales Sale  ON 
				  Sale.horse_id = Horse.id  LEFT JOIN tbl_horseforstuds Stud ON Stud.horse_id = Horse.id WHERE 1 AND Horse.approve_stat='Y'" ;
			if(@$_GET['gender']!="") {					
					$gender=$_GET['gender'];
					$sql.= " AND Horse.gender ='".$gender."'";		
					$pagelink.='&gender='.$gender;		
	...
	function myhorsesforstud() {
		...
		parent::chkusertype();
		$userid=$this->Session->Read("userid") ;		
		$listhorseforsale=$this->Horseforstud->FindAll('user_id='.$userid);
		$this->set('listhorseforsale',$listhorseforsale);
	}	
		...
		if(!empty($this->data)) {
			...
			if(!empty($this->data['Horse']['bred_name'])) {
				...
				if(count($stablecountarr)>0) {
					...
					$this->data['Changerequesthorse']['bred_name']='';
				}							
			}
