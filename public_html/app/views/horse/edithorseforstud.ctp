				...
				<div class="upper">
					...
					<div class="profile_info">
						...
						<div class="po_inf_mid">
							<div class="form_box">
								<p class="formarea" style="color: #994F26; font-size: 14px;"><strong>Put <?php e($useridarr[0]['Horse']['name']);?> up for stud</strong></p>
							</div>	
							<div class="form_box">
								<label class="formarea"><strong>Contact Details :</strong></label>
								<?php echo $form->textarea('Horsesale.contactdetails',array('rows'=>'4','cols'=>23,'value'=>$salearr['Horseforstud']['contact_details'])); ?>						
							</div>	
							<div>&nbsp;</div>
							
							<div class="form_box">
								<label class="formarea"><strong>Stud Details :</strong></label>
								<?php echo $form->textarea('Horsesale.salesdescription',array('rows'=>'4','cols'=>23,'value'=>$salearr['Horseforstud']['stud_details'])); ?>						
							</div>	
							<div>&nbsp;</div>		
									...
									if(is_array($pricerangearr)) {
										if(count($pricerangearr)>0) {
											foreach($pricerangearr as $key=>$val) :
												if($val['Pricerange']['id']==$salearr['Horseforstud']['pricerange_fromid']) {
							...
							<div class="form_box" style="margin-left: 20px;">
								...
								<select name="data[Horsesale][pricerange_toid]" class="dropdown98" id="pricerange">
									...
									if(is_array($pricerangearr)) {
										if(count($pricerangearr)>0) {
											foreach($pricerangearr as $key=>$val) :
												if($val['Pricerange']['id']==$salearr['Horseforstud']['pricerange_toid']) {
													$sel='selected=selected';
												}
												else {
													$sel='';
