...
<script language="javascript">
	...
	function subscribefornotification(horse_id) {
		if(horse_id) {
			document.getElementById("subsmsg").innerHTML="<font color=#FF0000>Please wait...</font><br><br>";
	...
	<div id="wrapper_parrent">		
		...
		<div class="sub_body">			
				<div class="upper">
					...
					<div class="po_inf_mid">
							...
							<label><strong>Sire :</strong></label>
									<?php											
											if($horsearr['Horse']['sire']!="") {												
												...
												e("<div align=left><em>You have not added any sire </em></div>");
											}
											?>
						...
						<div class="form_box10" style="float: left; width: 200px;">
							...
							<?php
											...
											if($horsearr['Horse']['dam']!="") {
												...
												e("<div align=left><em>You have not added any dam </em></div>");
											}
											?>						
							<div class="clear"></div>
						</div>
						<div class="form_box10" style="float: left; width: 200px;">
							<label><strong>Stable :</strong></label> <?php
												...
												?>
												Your horse is not listed in a stable<?php /*?>,  <a href="<?php e($html->url('/stable/create/'))?>" style="color:#994F26">create a stable page</a><?php */?>
												<?php
					...
					<div class="pannel0" style="position: absolute; width: auto; right: 0; top: -5px; margin-left: 0;" >
								<span id="subsmsg"></span><br>
								<span id="unsubsmsg"></span><br>
								<?php
								if(is_numeric($chksession)) {
								if($usertype=="P") {
								?>						
									<input  type="button"   value="Subscibe" class="newbutton" onClick="subscribefornotification('<?php e($horseid);?>')" style="cursor:pointer"/>
									&nbsp;&nbsp;
									<input  type="button"   value="Unsubscribe" class="newbutton" onClick="unsubsubscribefornotification('<?php e($horseid);?>')" style="cursor:pointer"/>
									&nbsp;&nbsp;
								<?php
								}
								if(count($horseowerarr)<=0) {
								?>
									<input  type="button"   value="Edit This Horse"  class="newbutton"   onClick="chkchangerequest('<?php e($horseid);?>')"/>
									<span id="chkrequestmsg"></span>
								<?php
								}
								else {
									if($usertype=="P") {
								?>
										<input  type="button"     value="Edit This Horse" class="newbutton"   onClick="window.location.href='<?php e($html->url('/horse/edithorseinfo/'.$horseid));?>'"/>
								<?php	
									}
									if($usertype=="F") {
									?>
										<input  type="button"     value="Edit This Horse"  class="newbutton"   onClick="window.location.href='<?php e($html->url('/horse/edithorseinfobyfreeuser/'.$horseid));?>'"/>
									<?php
									}		
								}														
								}
