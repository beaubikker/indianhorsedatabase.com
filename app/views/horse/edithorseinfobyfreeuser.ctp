...
<script type="text/javascript">	
	...
	function addtownvalid() {
			...
			if(err==0) {
				document.getElementById("showtownmsg").innerHTML="<font color=#FF0000>Adding wait....</font>"
	...
	function processrequestadditionaltown() {
		if(req.readyState==4)
			{				
				if(req.status==200)
				{			
					document.getElementById("showtownmsg").innerHTML="<font color=#FF0000>Added</font>"	;					
					listtownanother();		
					donemsg();										
				}
			}	
	...
	function assignmainimage(horse_id,image_id,image) {
		if(parseInt(image_id)) {		
			if(window.confirm("Are you sure that you want this as main image")) {
				window.location.href='<?php e($html->url('/horse/assignmainimage/'));?>'+horse_id+'/'+image_id+'/'+image
			}		
		}	
	}
...
<body onLoad="showmulimage();">		
	...
	<input type="hidden" name="hiddownerid" id="hiddownerid" value="<?php e($useridarr[0]['Horse']['ownerid']);?>">
					...
					<div class="profile_info">
						...
						<div style="float:left;">
							<div class="form_box">
								...
								<?php echo $form->text('Horse.name',array("class"=>"newinput","size"=>"30","onkeypress"=>"listingextinghorse()","autocomplete"=>"off","value"=>$useridarr[0]['Horse']['name'],"onchange"=>"chkhorse(".$horse_id.")","onclick"=>"blankallautocomplete()")); ?>
								<div align="right"><input type="checkbox" name="data[Horse][nameunknownoption]" value="Y" id="nameunknownoption" <?php if($useridarr[0]['Horse']['nameunknownoption']=="Y") { ?> checked="checked" <?php } ?> onClick="namedis()"><font color="#994F26"> Unknown</font></div>
							...
							<div class="form_box">
								...
								<select name="data[Horse][gender]" class="dropdown" size="1" id="gender"  onChange="blankallautocomplete()">
							...
							<div class="form_box">
								...
								<select name="data[Horse][breed_id]" class="dropdown" id="breed" onChange="blankallautocomplete()">
						  ...
						  <div class="form_box">
								...
								<input type="text" name="data[Horse][year]" id="year" value="<?php e($useridarr[0]['Horse']['year']);?>" size="30" onClick="blankallautocomplete()"> 
							</div>
							<?php 
						  ...
						  <div class="form_box" id="deathfield" style="margin-bottom: 10px; display:<?php e($show);?>;">
							...
							<div class="form_box">
								<label class="formarea">Sire:</label>								 
								  <?php echo $form->text('Horse.sire',array("size"=>"30","onkeypress"=>"listingextinghorsesire()","autocomplete"=>"off","value"=>$useridarr[0]['Horse']['sire'],"onclick"=>"blankallautocomplete()")); ?>
								 <br>
								 <span id="listsirehorse" style="overflow:visible;"></span>
								 <div class="clear"></div>
								 <div align="right"><input type="checkbox" name="data[Horse][sireunknowoption]" value="Y" id="sireunknowoption" <?php if($useridarr[0]['Horse']['sireunknowoption']=="Y") { ?> checked="checked" <?php } ?> onClick="disablesire()"><font color="#994F26"> Unknown</font></div>
							</div>
							
							<div class="form_box">
								<label class="formarea">Dam:</label>
								 <br>
								<?php echo $form->text('Horse.dam',array("size"=>"30","onkeypress"=>"listingextinghorsedam()","autocomplete"=>"off","value"=>$useridarr[0]['Horse']['dam'],"onclick"=>"blankallautocomplete()")); ?>
								 
								 <br><span id="listsiredam" style="overflow:visible;"></span>
								 <div class="clear"></div>
								 <div align="right"><input type="checkbox" name="data[Horse][damunknownoption]" value="Y" <?php if($useridarr[0]['Horse']['damunknownoption']=="Y") { ?> checked="checked" <?php } ?> onClick="disabledam()" id="damunknownoption"><font color="#994F26"> Unknown</font></div>
								<div class="clear"></div>
							</div>
							
							<div class="form_box">
								<label class="formarea">Height:</label> 
								<select name="data[Horse][height_id]" class="dropdown"  id="height" onChange="blankallautocomplete()">
							...
							<div class="form_box">
								<label class="formarea">Coat color:</label>
									<select name="data[Horse][coatcolor_id]" class="dropdown" id="coatcolor_id" onChange="blankallautocomplete()">
							...
							<div class="form_box" id="regcode" style="margin-bottom: 10px; display:<?php e($showregister);?>">
								<label class="formarea"><strong style="padding-left: 10px;">Number</strong></label>								
                                 	<input type="text" name="data[Horse][registration_code]" id="registrationcode" size="30" value="<?php e($useridarr[0]['Horse']['registration_code']);?>" onChange="blankallautocomplete()">                                 
							 </div>	
							
							
							<div class="form_box">
								<label class="formarea">Bloodline:</label><?php echo $form->text('Horse.bloodline',array("size"=>"30","value"=>$useridarr[0]['Horse']['bloodline'],"onclick"=>"blankallautocomplete()")); ?>						
								<div class="clear"></div>
							</div>
							<div class="form_box">
								<label class="formarea">Breeder:</label><?php echo $form->text('Horse.breeder',array("size"=>"30","onkeypress"=>"listmemberall()","autocomplete"=>"off","value"=>$useridarr[0]['Horse']['breeder'],"onclick"=>"blankallautocomplete()")); ?>
								 <br>
							...
							<div class="form_box">
								...
								<?php echo $form->text('Horse.ownername',array("size"=>"30","onkeypress"=>"listmemberowner()","autocomplete"=>"off","value"=>$useridarr[0]['Horse']['ownername'],"onclick"=>"blankallautocomplete()")); ?>
								 <br>
							...
							<div class="form_box">
								...
								<?php echo $form->text('Horse.stablename',array("size"=>"30","onkeypress"=>"liststable()","autocomplete"=>"off","value"=>$stablename,"onclick"=>"blankallautocomplete()")); ?>
								 <br>
							...
							<div class="form_box">
								...
								<div align="center" style="width:665px; display:none" id="showtownmsg"><b><font style="color:#000000; padding-left:1px;">Select town drop down -If it's not listed add your town(make sure to spell it right)</font></b></div>	
								<div class="clear"></div>														
							</div>
								
							</span>
							
							<div class="form_box" id="addtown" style="display:none">
								<label class="formarea">Add Town</label>								
								<br>	
								<?php echo $form->text('Horse.addadditionaltown',array("size"=>"30","onclick"=>"blankallautocomplete()","id"=>"addadditionaltown")); ?>	
								<input class="mass00" type="button" value="Add" id="valid"  onClick="addtownvalid()" style="cursor:pointer"/>
							...
							<div class="form_box">
								...
								<?php echo $form->text('Horse.bred_name',array("size"=>"30","onkeypress"=>"listbred()","autocomplete"=>"off","value"=>$useridarr[0]['Horse']['bred_name'],"onclick"=>"blankallautocomplete()")); ?>
								 <br>
							...
							<div class="form_box" style="height: 50px;">
								<label class="formarea">Prize won:</label><?php echo $form->textarea('Horse.prize_won',array('rows'=>'4','cols'=>'50','style'=>'border: 1px solid #F2CE87; position: absolute; height: 40px',"value"=>$useridarr[0]['Horse']['prize_won'],'class'=>'newtextarea',"onclick"=>"blankallautocomplete()")); ?>
								<div class="clear"></div>
							</div>		
							
							
							<div class="form_box" style="height: 150px;">
								<label class="formarea">Other details:</label><?php echo $form->textarea('Horse.other_details',array('rows'=>'8','cols'=>23,"class"=>"newtextarea","value"=>$useridarr[0]['Horse']['other_details'],"onclick"=>"blankallautocomplete()")); ?>				
								<div class="clear"></div>
							</div>		
...
<div style="padding-left:15px"><input type="hidden" id="num" value="8" size="2" name="num" class="file23"></div>
								...
								<div style="width: 680px; text-align: left; margin-bottom: 40px;">
									<?php
									...
									<?php								
										if($image) {
											...
											?>
												...
												<br>
												<b><input type="checkbox" name="delmainimage"  value="Y"><img  src="<?php e($this->webroot);?>img/del.png" alt="" align="absbottom"   title="delete" style="cursor:pointer" >Main Image </b>
											<?php
											}
										}
										?>										
								...
								if(count($additionalimagearr)>0) {
									...
									if(is_array($additionalimagearr)) {
										...
										if($image!="") {
											if(file_exists(rootpth()."/".$imagedirectory."/".$image)) {
												$xy = $rsz->imgResize(rootpth()."horseadditionalimage/".$val['Horseimage']['image'],120,80);
											?>
												<div <?php if($cntr%3!=0) { ?> style="width: 200px; float: left; margin: 0 40px 40px 0;" <?php } else { ?> style="width: 200px; float: left; margin: 0 0 40px 0;" <?php } ?>>
												<img  src="<?php e($this->webroot);?>img/horseadditionalimage/<?php e($image);?>" alt="" align="absbottom"  width="<?php e($xy[0]);?>" height="<?php e($xy[1]);?>"/>
												
												<br><input type="radio" name="mainimg[]"  value="<?php e($val['Horseimage']['id']);?>"> &nbsp;<b>Main Image </b>
												<input type="checkbox" name="delimage[]"  value="<?php e($val['Horseimage']['id']);?>"><img  src="<?php e($this->webroot);?>img/del.png" alt="" align="absbottom"   title="delete" style="cursor:pointer" >
