...
?>    
 <script type="text/javascript">			
	...
	function deladdimage(image,id) {
		if(parseInt(id)) {
			window.location.href='<?php e($html->url('/stable/addiimagedel/'));?>'+image+'/'+id;
		}	
	}		
</script>  
<script type="text/javascript">
		...
		function chkstable(stableid) {
			...
			if(StableStableName!=""){
				...
				if(req)
				{
					...
					req.open("GET","<?php echo $html->url('/stable/chkstableforedit/');?>"+StableStableName+"/"+stableid,true);	
					req.send(null);					
				}
			}
		}		
	...
	<div id="wrapper_parrent">		
		...
		<div class="sub_body">			
				<div class="upper">
					...
					<?php e($form->hidden('Stable.edited_date',array("value"=>date('Y-m-d')))); ?>	
					<div class="profile_info">
						...
						<div style="float:left;">
							<div class="form_box">
								...
								<?php echo $form->text('Stable.stable_name',array("size"=>"30","value"=>$stablearr['Stable']['stable_name'],"class"=>"newinput","onchange"=>"chkstable(".$stable_id.")")); ?>
										...
										<?php
											...
											foreach($listhorst as $key=>$val) :
													...
													<td valign="top">	
														...
														if($image!="") {
															if(file_exists(rootpth()."/".$imagedirectory."/".$image)) {
																...
																?>									
																<img src="<?php e($this->webroot);?>img/horseimage/<?php e($image);?>" alt="" width="<?php e($xy[0]);?>"  height="<?php e($xy[1]);?>" align="absmiddle">
															<?php
															}
															else {
																?>
															...
															?>
																<img src="<?php e($this->webroot);?>img/noimage.jpg" alt="" width="60" height="60">																<?php
														}
														?>												</td>
												...
												//if($val['Horse']['stable_id']>0) {
													if($val['Horse']['stable_id']==$stable_id) {?>
														 <input type="checkbox" name="data[Stable][horse_id][<?php echo $key ;?>]" value="<?php echo $val['Horse']['id'] ;?>" id="<?php echo $val['Horse']['id'] ;?>"  checked="checked"/>
													<?php
													}
													elseif(!is_numeric($val['Horse']['stable_id'])) {?>
...
style="width: 10px;" class="jScrollPaneDragBottom"></div></div></div><div
 ...
 ?>
							...
							<div class="form_box">
								...
								<?php echo $form->text('Stable.website',array("size"=>"30","value"=>$stablearr['Stable']['website'])); ?>
								<div class="clear"></div>
							</div>	
							<div class="form_box">
								<label class="formarea">Stable Image:</label> 
								...
								<?php
									...
									if($image!="") {
										if(file_exists(rootpth()."/".$imagedirectory."/".$image)) {
											$xy = $rsz->imgResize(rootpth()."stable_image/".$stablearr['Stable']['stable_image'],250,135);						
											?>									
											<img src="<?php e($this->webroot);?>img/stable_image/<?php e($image);?>" alt="" width="<?php e($xy[0]);?>" align="absmiddle"  height="<?php e($xy[1]);?>">
										<?php
										}
										else {
											?>
											<img src="<?php e($this->webroot);?>img/noimage.jpg" alt="" width="100" height="100">
											<?php
										}
									}
									else {
								...
								<select name="data[Horse][countryid]" class="dropdown" id="HorseCountry" onChange="liststate()">
									...
									if(is_array($country_arr)) {
										foreach($country_arr as $key=>$val) :	
											if($val['Country']['id']==$stablearr['Stable']['country_id']) {
							...
							<div class="form_box">
								<label class="formarea">About:</label> <?php echo $form->textarea('Stable.about',array('rows'=>'4','cols'=>23,"value"=>$stablearr['Stable']['about'])); ?>								
							</div>					
							<div class="form_box">
								<label class="formarea">Services:</label><?php echo $form->textarea('Stable.services',array('rows'=>'8','cols'=>23,"value"=>$stablearr['Stable']['services'])); ?>				
							</div>							
							<div class="form_box">
							...
							<div class="upload">
								...
								if(count($mulimage_arr)>0) {
									if(is_array($mulimage_arr)) {
										...
										if(file_exists(rootpth()."/".$imagedirectory."/".$image)) {
											$xy = $rsz->imgResize(rootpth()."multiplestableimage/".$val['Stableimage']['image'],80,60);
										?>
											<img class="upload50" src="<?php e($this->webroot);?>img/multiplestableimage/<?php e($image);?>" alt="" align="absbottom"  width="<?php e($xy[0]);?>" height="<?php e($xy[1]);?>"/>
											<img class="upload50" src="<?php e($this->webroot);?>img/del.png" alt="" align="absbottom"   title="delete" style="cursor:pointer" onClick="deladdimage('<?php e($image);?>','<?php e($val['Stableimage']['id']);?>')"/>											
										<?php
										}
										endforeach;
									}
    ...
    <div class="pop_up" style="z-index:50000000000; background-color:#FFFBEC; border:#DBCB90 solid 1px;">
									<h3 class="pop_up_h" style="color:#994F26; font-family:verdana; font-size:16px; padding:12px 15pxs 0 20px; display:inline; padding-left:20px; ">Upload Stable images</h3>
									<a href="javascript:void(0)" onClick="closepic()">Close&nbsp;<strong>x</strong></a>
									<p class="pop_up_p" style="color: #343434; font-family: verdana; font-size: 12px; line-height: 20px; padding: 10px 0 10px 20px; width:466px;">The Main image is used to represent your stable throughout the Database. Please make sure to select your best one.</p>
