...

{
	...
	function listlatesthorseforsale() {		
		...
		$listhorseforsale_sql="SELECT * FROM tbl_horses Horse,tbl_horsesales Horsesale WHERE Horse.approve_stat='Y' 
							   AND Horsesale.horse_id=Horse.id AND Horsesale.salesfor='Sale' ORDER BY Horsesale.id DESC LIMIT 0,10 ";

		//$horsearr= $this->Horse->findAll("approve_stat='Y' AND sales_status='S'",'','order by id desc','','LIMIT 0, 10'); 
		$horsearr=$this->Horse->query($listhorseforsale_sql) ;
		...
		if(!empty($this->data)) {
			...
			if(isset($_POST['delimage'])) {
				...
				foreach($_POST['delimage'] as $key=>$value) {
					...
					$additioimage_sql="SELECT * FROM tbl_horseimages Horseimage WHERE id=".$id ;
			...
			if(isset($_POST['delmainimage'])) {
				$this->Horse->id=$horse_id;
				$this->data['Horse']['image']='';
				$this->Horse->save($this->data);
			}
			$this->redirect('/horse/mylistedhorse');
	...
	function viewchange($horse_id=NULL) { 
		...
		$addionalimagechange_sql="SELECT * FROM tbl_horseimages ChangerequestHorseimage WHERE horse_id =".$horse_id." AND image!='' AND changed_from='N'"; ;
		$changereqarr=$this->Horse->query($addionalimagechange_sql) ;
		$this->set('changereqarr',$changereqarr);
