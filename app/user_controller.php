...
class UserController  extends AppController
{
	...
	function freeuserwelcomepage() {
		...
		$changeeditnotofication_sql="SELECT * FROM tbl_horses Horse, tbl_changerequesthorses Changerequest 
									...
									Horse.ownerid=".$userid." AND Changerequest.changed_date>='".$userdetailarr['User']['logoutdate']."' GROUP BY Changerequest.requestedby_id "; 
		...
		$breedernotification_sql="SELECT * FROM tbl_horses Horse, tbl_changerequesthorses Changerequest 
									WHERE Horse.id=Changerequest.horse_id AND Changerequest.approve_status='Y' AND 
									Horse.breeder_id=".$userid." AND Changerequest.changed_date>='".$userdetailarr['User']['logoutdate']."' GROUP BY Changerequest.requestedby_id "; 
		...
		$addernotification_sql="SELECT * FROM tbl_horses Horse, tbl_changerequesthorses Changerequest 
									WHERE Horse.id=Changerequest.horse_id AND Changerequest.approve_status='Y' AND 
									Horse.addedby=".$userid." AND Changerequest.changed_date>='".$userdetailarr['User']['logoutdate']."' GROUP BY Changerequest.requestedby_id "; 
	...
	function noaccess() {
		parent::blanklayout();
		$homecontent_arr=$this->Content->FindAll("pagename  LIKE '%NoAccess%'");
	...
	function paiduserlogout() {
		...
		$homecontent_arr=$this->Content->FindAll("pagename  LIKE '%Log Out%'");
		$this->set('homecontent_arr',$homecontent_arr);
	}
	...
	function confirmupdatemembership() {
		...
		$this->data['User']['expiration_time_periode']=$this->data['User']['expiration_time_periode'] ;
		$durationexpiration=strtotime("+ ".$this->data['User']['expiration_time_periode'] );
		$membershipexpiration=date("Y-m-d",$durationexpiration);
		$this->data['User']['membership_exipre_date']=$membershipexpiration;
		$this->data['User']['payment_option']=$this->data['User']['payment_option'] ;
		$this->data['User']['payment_status']='N';
		$this->User->save($this->data);	
	...
	function paymentformemshipextension() {
		...
		$this->data['User']['usertype']='P';
		$this->data['User']['expiration_time_periode']=$this->data['User']['expiration_time_periode'] ;
		$curreexpiration=$userarr['User']['expiration_time_periode'] ;
	...
	function signup($paidmember=NULL,$login=NULL)  {
		parent::blanklayout();
		if($paidmember=="") {
			$this->redirect('/user/selectmembership');
		}
		$seetingarr=$this->Setting->FindByid(1);
		$this->set('seetingarr',$seetingarr);	
		if(!empty($this->data)) {
	...
	function profile($msg=NULL) {
		...
		$breddnamearr=$this->Horse->FindAll("approve_stat='Y' AND breed_id=".$userid." LIMIT 0,7 "); 
		...
		if(count($breddnamearr)>0) {
			$allbreddnamearr=$this->Horse->FindAll("approve_stat='Y' AND breed_id=".$userid); 
	...
	function freeuserprofile($msg=NULL) {
		...
		$userarr=$this->User->FindById($userid)	;	
		$breedname=$userarr['User']['firstname']." ".$userarr['User']['lastname'] ;	
		$ownerhorse_arr=$this->Horse->FindAll("approve_stat='Y' AND ownername='".$breedname."' LIMIT 0,7 "); 
		$this->set('ownerhorse_arr',$ownerhorse_arr);
		if($userarr['User']['usertype']=="P") {
			$stablenamearr=$this->Stable->FindAll('userid='.$userid);
			$this->set('stablenamearr',$stablenamearr);
		...
		if(count($ownerhorse_arr)>0) {
			$ownerhorse_allarr=$this->Horse->FindAll("approve_stat='Y'AND ownername='".$breedname."'"); 
			$this->set('ownerhorse_allarr',$ownerhorse_allarr);
		}
		$breddnamearr=$this->Horse->FindAll("approve_stat='Y' AND breeder='".$breedname."' LIMIT 0,7 "); 
		$this->set('breddnamearr',$breddnamearr);
		if(count($breddnamearr)>0) {
			$allbreddnamearr=$this->Horse->FindAll("approve_stat='Y' AND breeder='".$breedname."'"); 
			$this->set('allbreddnamearr',$allbreddnamearr);
		}
		$this->set('userarr',$userarr);
		$this->set('msg',$msg);
	...
	function paymodeuser($duration=NULL,$payoption=NULL) {
		...
		if($this->User->save($this->data)) {
			...
			<a href=http://indianhorses.india-web-design.com/user/confirmlink/'.base64_encode($userid).'>Click Here </a> To activate your account <br>
	...
	function addfreeuser($firstname=NULL,$lastname=NULL,$email=NULL,$password=NULL) {
		...
		if(count($userarr)>0) {
			...
			$adminArr = $this->Admin->FindBySuperAdmin(1);	
			$textContent2='';
			$textContent2='<table bgcolor=\"#feefcd\" height=\"700px\" width=\"700px\">
	<tr>
		<td height=\"156\">
			<p align=\"center\"><img src=\"http://indianhorsedatabase.com/img/settingimages/1307821381_IHD-Logo(final)beta.png\" alt=\"\" width=\"456\" height=\"156\"></p>
		</td>
	</tr>
	<tr>
		<td height=\"100\">
			<h2 align=\"center\">You are now part of the IHD</h2>
		</td>
	</tr>
	<tr>
		<td>
			<p align=\"justify\" style=\"font-weight:bold; margin-left:70px; margin-right:30px;\">
				Hi '.$firstname.' '.$lastname.'<br><br>
You have successfully registered as free user with Indian Horse Database<br><br>
<a href=http://indianhorses.india-web-design.com/user/confirmlink/'.base64_encode($userid).'>Click Here</a> to activate your account.<br><br>
This is your login information: <br>
...
Password      :'.$password.'</b>
	...
	<tr>
		<td> 
			<p style=\"margin-left: 390px;\">
				<a target=\"blank\" href=\"http://www.indianhorsedatabase.com/user/newuser\"><img src=\"http://www.reyeng.com/images/IHD/button_for_email.png\"></a>			
				</p>
		</td>
	</tr>
	<tr>
	...
	<tr>		
		<td bgcolor=\"#fffbec\" height=\"40\">
			<p align=\"center\" style=\"margin:10px;\">
				You are receiving this email because you have registered with <a target=\"blank\" href=\"http://www.indianhorsedatabase.com/\">www.indianhorsedatabase.com</a>
			</p>
		</td>
	</tr>
</table>';		
	...
	function confirmlink($userid=NULL) {
		...
		<br>
		<a href=http://indianhorses.india-web-design.com/user/signup/paidmember/loginhere>Click Here </a> To Login 
		<br>
		Thanks
		Indian Horse Database';		
		$textContent2 = stripslashes($textContent2);						
			...
			if(isset($_REQUEST['tx'])) {
				...
				if($type != ''){
					if($type=="premuim") {
						...
						//Send as 'html', 'text' or 'both' (default is 'text')
						if($this->Email->send()) {
							$this->redirect('/user/purchasesuccess');
						}					
					}			
				}
			}	
			...
			if(isset($_REQUEST['tx'])) {
				...
				if($type != ''){
					if($type=="upgradepremuim") {
						...
						$this->Session->destroy("username") ;	
						$this->Session->destroy("email_address") ;	
						$this->Session->setFlash('You have successfully purchased premium membership service .Please Login Again');
						$this->Email->send() ;
						$this->redirect('/content/front');									
					}			
				}
	...
	function cancelpremiummembership() {
		...
		echo $update_sql="UPDATE tbl_users SET usertype='F' AND payment_status='N'  WHERE id=".$userid ;
